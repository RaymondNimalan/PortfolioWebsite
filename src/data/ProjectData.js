import InfuaxWarsMockup from '../images/InfauxMockup.jpeg';
import PoshPupsMockup from '../images/PoshPupsMockup.jpeg';
import CloneGPTMockup from '../images/CloneGPTMockup.png';
export const projectData = [
  {
    title: 'CloneGPT',
    about1: 'A JavaScript and Node.js clone of the famous A.I chatbot ChatGPT',
    about2: 'Tech Stack: JavaScript, React, Express, Node.js, OpenAI API',
    image: CloneGPTMockup,
    websiteUrl: 'TBD',
    githubUrl: 'https://github.com/RaymondNimalan/clonegpt',
  },
  {
    title: 'MaskOn',
    about1:
      'Responsive web application using label detection through machine learning to verify if a person is wearing a mask to prevent the spread of COVID-19.',
    about2:
      'Tech Stack: JavaScript, React, Redux, Express, Node.js, Google Vision API',
    image: InfuaxWarsMockup,
    websiteUrl: 'http://mask-on-app.herokuapp.com/',
    githubUrl: 'https://github.com/RaymondNimalan/MaskOn',
  },
  {
    title: 'Infaux Wars',
    about1:
      'Progressive web application using Google machine learning to determine the reliability of online news articles.',
    about2:
      'Tech Stack: Google NL API, JavaScript, Python, Node.js, Express, React, Redux, Sequelize, PostgreSQL',
    image: InfuaxWarsMockup,
    websiteUrl: 'https://infaux-wars.herokuapp.com/',
    githubUrl: 'https://github.com/2011-sagittarius/FakeNews',
  },
  {
    title: 'Posh Pups',
    about1:
      'Fully functioning e-commerce web application selling clothing for dogs',
    about2:
      'Tech Stack: JavaScript, Node.js, Express, React, Redux, Sequelize, PostgreSQL',
    image: PoshPupsMockup,
    websiteUrl: 'http://posh-pups.herokuapp.com/',
    githubUrl: 'https://github.com/Team-Beta-GraceShopper/GraceShopper',
  },
];
